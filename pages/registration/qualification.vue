...
export default {
  mounted() {
    this.$store.commit("app/SET_TITLE", "Qualification Information");
    if (
      this.user.registered_stages &&
      !(
        this.user.registered_stages.includes("qualification-information") ||
        this.user.next_stage === "qualification-information"
      )
    ) {
  ...
  data() {
    return {
      ...
      shouldLoadEducationHistory: false,
      shouldQualificationInformation: true,
