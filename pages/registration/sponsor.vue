...
export default {
  mounted() {
    this.$store.commit("app/SET_TITLE", "Fees payer information");
    if (
      this.user.registered_stages &&
      !(
        this.user.registered_stages.includes("sponsor-information") ||
        this.user.next_stage === "sponsor-information"
      )
    ) {
  ...
  data() {
    return {
      ...
      // shouldLoadSponsorInformation: true,
