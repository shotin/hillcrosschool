...
export default {
  mounted() {
    this.$store.commit("app/SET_TITLE", "Document Upload");

    if (
      this.user.registered_stages &&
      !(
        this.user.registered_stages.includes("document-uploads") ||
        this.user.next_stage === "document-uploads"
      )
    ) {
